 ...
 */
contract PrexPaymaster is BasePaymaster {
    ...
    bool public bundlerAllowed;
    address allowedBundler;

    event PaymasterLog(
        uint32 spentKey,
        uint96 gasCost,
        uint96 cumulativeSpent
    );
     ...
     */
    function getHash(UserOperation calldata userOp, uint48 validUntil, uint48 validAfter, uint32 spentKey)
     ...
     * paymasterAndData[36:101] : signature
     */
    function _validatePaymasterUserOp(UserOperation calldata userOp, bytes32 /*userOpHash*/, uint256 requiredPreFund)
    internal view override returns (bytes memory context, uint256 validationData) {
        (uint48 validUntil, uint48 validAfter, uint32 spentKey, bytes calldata signature) = parsePaymasterAndData(userOp.paymasterAndData);
        ...
        bytes32 hash = ECDSA.toEthSignedMessageHash(getHash(userOp, validUntil, validAfter, spentKey));
        ...
        return (_packContextData(userOp, spentKey), _packValidationData(false, validUntil, validAfter));
    }

    function _packContextData(UserOperation calldata userOp, uint32 spentKey) internal pure returns (bytes memory) {
        return abi.encode(userOp.maxFeePerGas, userOp.maxPriorityFeePerGas, spentKey);
    }

    function _postOp(PostOpMode mode, bytes calldata context, uint256 actualGasCost) internal override {
        (uint256 maxFeePerGas, uint256 maxPriorityFeePerGas, uint32 spentKey) = abi.decode(context, (uint256, uint256, uint32));

        // unfortunately tx.origin is not allowed in validation, so we check here

        require(allowedBundler == address(0) || allowedBundler == tx.origin, "Paymaster: bundler not allowed");
        ...
        if (mode != PostOpMode.postOpReverted) {
            ...
            uint96 gasCost = uint96(actualGasCost + overhead*gasPrice);

            spent[spentKey] += gasCost;

            emit PaymasterLog(spentKey, gasCost, spent[spentKey]);
        }
    }

    function parsePaymasterAndData(bytes calldata paymasterAndData)
    internal pure returns(uint48 validUntil, uint48 validAfter, uint32 spentKey, bytes calldata signature) {
    ...
    function setBundler(address bundler) public onlyOwner {
        allowedBundler = bundler;
    }
